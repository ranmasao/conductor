...
Conductor is a local deterministic orchestrator for agent-driven software
...
Conductor uses three Git surfaces:

- The operator checkout contains the product branch.
- A Conductor-owned control worktree contains canonical workflow tickets and
  execution reports.
- A Conductor-owned execution worktree contains the selected ticket's worker
  changes and durable execution branch.

Git remains the canonical history for product and control changes. Local
operational runtime state is separate: Conductor stores one opaque state record
in a SQLite database under `STATE_DIR` (by default
`$XDG_STATE_HOME/conductor`, or `~/.local/state/conductor`). The database name
...
incomplete `.conductor/project.md` project-context adapter. It does not start
...
The project owns `.env`, `.conductor/project.md`, its templates, and rendered
artifacts. Conductor preserves existing project files. Project context names
...
conductor reconcile update-base <ticket-id> --onto <branch>
```
...
what Conductor would decide from one consistent snapshot and does not mutate
runtime state.
...
operation is ambiguous, Conductor fails closed and keeps the evidence for
...
- The product checkout and control worktree must be clean before synchronization
  or scheduling. Conductor does not destroy local changes or reconcile
...
- The configured control and workflow paths are validated before use. Only
  ...
  not tickets.
- `CONTROL_BRANCH` defaults to `conductor/control`. Its worktree is under
  `STATE_DIR/worktrees/<repository-key>/control`; status, plan, check, and run
  do not create it as a side effect of observation.
- Each selected ticket uses a durable `conductor/work/<ticket-id>` execution
  branch for lineage and history, plus a disposable Conductor-owned execution
...
- Ticket frontmatter uses restricted NanoYAML. Conductor validates metadata,
...
- Tickets move through `backlog -> todo -> review -> accepted -> done`.
  Conductor selects one runnable ticket at a time and does not treat review or
...
- Worker output is untrusted. Only one strictly validated `conductor_report`
...
This creates `.venv`, installs Conductor in editable mode, and installs the
...
./dev run [args...]     Run the installed Conductor CLI
...
Conductor 0.5 remains an unreleased development architecture. Background
