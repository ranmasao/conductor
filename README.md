...
`conductor init` may make the product checkout dirty by creating `.env`, the
`.conductor/project.md` adapter, project-owned templates, and generated role
...
workflow. Conductor does not commit, push, open reviews, choose a target branch, or
edit `.gitignore` for that workflow. Before `conductor control init` and workflow
...
Conductor only validates the resulting checkout. The `0.4.0` release includes the
...
run `conductor render` when upgrading an older project.
...
- Ticket frontmatter uses the restricted [NanoYAML implementation](src/nanoyaml/); Conductor does not depend on a general YAML library or support free-form tickets.
...
- Conductor owns worker prompt construction. The worker receives one exact implementation assignment and a small worker-only contract. Canonical workflow paths, ticket paths, kanban, and Git/execution topology are Conductor details, not worker API.
...
- Pre-worker synchronization transactions may resume when their result can be proven from persisted state and Git observations. If Conductor is interrupted after worker execution enters `agent_running`, v0.4 fails closed and does not automatically repeat or reconcile that execution. General execution recovery is outside the v0.4 scope.
