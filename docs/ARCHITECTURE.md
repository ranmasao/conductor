...
Conductor keeps product work, workflow records, and active ticket work in
separate places.

- The operator checkout contains the product branch and product code.
- The control branch and its Conductor-owned worktree contain workflow tickets,
  workflow directories, and execution reports.
- Each selected ticket has a durable execution branch and a Conductor-owned
...
By default the state is under `$XDG_STATE_HOME/conductor`, or
`~/.local/state/conductor` when `XDG_STATE_HOME` is unset. `STATE_DIR` can
...
`conductor reconcile control --from <local-head> --to <remote-head>` is not
...
acceptance moves the ticket to `accepted`; Conductor then integrates the
...
uses proven fast-forward ancestry only. Conductor does not automatically rebase,
merge conflicting histories, resolve conflicts, or perform destructive
reconciliation.

## Worker Boundary

Workers edit implementation files and return a result or claim. Conductor owns:
...
`conductor_report` event. Missing, malformed, duplicate, or technically failed
...
work is accepted; Conductor performs accepted product integration.
...
- Worktrees are recreated only after branch and workspace identity is proven.
  Conductor does not reset, force-remove, or steal an unsafe worktree.
...
Project-side `.env`, `.conductor/project.md`, templates, rendered role
artifacts, and existing documentation remain owned by the target project.
`conductor init`, `render`, and `control init` prepare or validate those
