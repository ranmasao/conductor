...
Conductor keeps three physically separate Git surfaces:
...
- A Conductor-owned control worktree contains canonical workflow tickets and
  execution reports.
- A Conductor-owned execution worktree contains the selected ticket's changes
  and durable execution branch.

Git is the canonical history for product and control changes. Local operational
runtime state is separate. Conductor stores one opaque state record in a SQLite
database under `STATE_DIR` (by default `$XDG_STATE_HOME/conductor`, or
`~/.local/state/conductor`). The database name is derived from repository
identity and is not workflow history.

The control branch defaults to `conductor/control`. Its worktree is under
`STATE_DIR/worktrees/<repository-key>/control`. A selected ticket uses the
durable `conductor/work/<ticket-id>` branch for lineage and history, plus a
...
fast-forward ancestry only. Conductor does not automatically rebase, merge
...
`accepted` do not satisfy dependencies. Conductor selects one runnable ticket
...
acceptance moves it to `accepted`, and Conductor integrates accepted work
before moving it to `done`.

The product checkout and control worktree must be clean before synchronization
or scheduling. Conductor validates configured control and workflow paths and
...
recreated only when branch and workspace identity can be proven; Conductor
...
Ticket frontmatter uses the restricted NanoYAML implementation. Conductor
...
Conductor owns worker prompt construction. Workers receive one implementation
assignment and a small worker-only contract, not control-worktree paths,
workflow topology, execution IDs, or branch identity.

Worker output is untrusted. Only one strictly validated `conductor_report` tool
event can provide a semantic claim. Missing, malformed, duplicate, or
technically failed output does not mark work completed. Conductor keeps worker
...
product code; product integration is Conductor-owned.
