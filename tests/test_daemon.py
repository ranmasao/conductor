...
from conductor.runtime import ShutdownInterrupted, WorkflowBlockedError
...
def test_foreground_service_command_constructs_one_service_engine(
    ...
    monkeypatch.setattr(
        cli.sys,
        "argv",
        ["conductor", "--foreground", "--env", str(config)],
    )
    monkeypatch.chdir(working)
...
def test_service_survives_recoverable_stage_conductor_error(
    ...
    def iteration():
        ...
        if len(calls) == 1:
            ...
            raise ConductorError("simulated publication transport failure")
        stop_event.set()
        return 0
...
def test_recoverable_stage_error_keeps_once_semantics(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        engine,
        "_run_once",
        lambda: (_ for _ in ()).throw(ConductorError("one-shot failure")),
    )

    with pytest.raises(ConductorError, match="one-shot failure"):
        daemon.run_service(engine, once=True)


def test_unrelated_conductor_error_still_escapes_service(tmp_path, monkeypatch):
    engine, _config, _state = make_engine(tmp_path, monkeypatch)
    monkeypatch.setattr(
        engine,
        "_run_once",
        lambda: (_ for _ in ()).throw(ConductorError("fatal invariant")),
    )

    with pytest.raises(ConductorError, match="fatal invariant"):
...
def test_merge_pending_retries_matching_control_merge(tmp_path, monkeypatch):
    ...
    git(publisher, "switch", "--track", "origin/conductor/control")
    git(publisher, "config", "user.name", "Conductor Test")
    git(publisher, "config", "user.email", "conductor@example.test")
    (publisher / "control-change.txt").write_text("control\n")
    git(publisher, "add", "control-change.txt")
    git(publisher, "commit", "-m", "control change")
    target = git(publisher, "rev-parse", "HEAD").stdout.strip()
    git(publisher, "push", "origin", "HEAD:refs/heads/conductor/control")
    local = git(engine.repo, "rev-parse", "HEAD").stdout.strip()
    control = git(engine.control_worktree, "rev-parse", "HEAD").stdout.strip()
    engine._save_state(
        "merge_pending",
...
def test_explicit_retry_sigterm_owns_worker_process_group(tmp_path, monkeypatch):
    ...
    daemon_process = subprocess.Popen(
        [sys.executable, "-m", "conductor", "--foreground", "--env", str(config)],
        cwd=working,
        env=environment,
        text=True,
...
def test_sigkill_parent_and_retry_refuses_duplicate_worker(tmp_path, monkeypatch):
    ...
    worker.write_text(
        ...
        "child = subprocess.Popen([sys.executable, '-c', "
        "'import time; time.sleep(60)'])\n"
        "with pathlib.Path(os.environ['CONDUCTOR_TEST_MARKER']).open('a') as file:\n"
...
def test_natural_leader_exit_with_live_descendant_blocks_post_worker(
    ...
    worker.chmod(0o755)
    monkeypatch.chdir(working)
    monkeypatch.setenv("CONDUCTOR_TEST_MARKER", str(marker))
    ...
    try:
        with pytest.raises(
            ConductorError, match="process group is still alive"
        ):
            engine.run_once()
        assert marker.exists()
        processes = json.loads(marker.read_text())
        ...
        with pytest.raises(ConductorError, match="ownership is indeterminate"):
            restarted.run_once()
        assert os.kill(processes["child"], 0) is None
