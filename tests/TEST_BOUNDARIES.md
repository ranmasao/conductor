...
| `Application` | `cli._service_engine()` imports `conductor.application` and checks `application.Application`; the alias currently resolves to `ServiceEngine` but the branch permits an intentional compatibility implementation | KEEP - supported compatibility alias/seam | `src/conductor/application.py`, `src/conductor/cli.py:48-55` |
| `ConductorApplication` | Exported as an importable name from `conductor.application`; no current in-repository caller | KEEP - compatibility surface, reachability externally unclear | Explicit alias declaration; absence from local callers does not disprove supported imports |
...
- `test_service_engine_is_the_only_runtime_authority` checks the current
  `Application`/`Conductor` identity aliases.
...
audit above now resolves `Application`, `Conductor`, `ServiceEngine`, `run`,
...
protected contract is proven retired. In particular, `Conductor` is still
...
- `test_service_engine_is_the_only_runtime_authority`: compatibility identity
  assertion; retain while `Application` and `Conductor` aliases are reachable.
...
- `ConductorApplication` has no local caller, but its explicit importable alias
...
- Whether `ConductorApplication` is a supported external import or an
