...
This document is the test-proof map for Conductor 0.5.0. It is the source of
...
| Production topology | `LiveService` starts `python -m conductor run`, real CLI subprocesses use the real socket, or `h1_driver.py` starts the real service engine | Daemon authority, independent CLI/service lifetime, real socket lifecycle, process identity, crash/restart and persisted recovery | Nothing beyond the scenario and assertions actually made |
