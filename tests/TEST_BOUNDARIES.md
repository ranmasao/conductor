...
This document is the maintained test-proof map for the current Conductor 0.5
...
| Engine semantic | `ServiceEngine`/`Conductor` constructed directly; direct methods or `serve()` in a test thread | State-machine decisions, admission rules, recovery algorithms, Git/state invariants, worker-result handling | Independent CLI process lifetime or OS crash topology |
...
| Production topology | `LiveService` starts `python -m conductor --foreground`, real CLI subprocesses use the real socket, or `h1_driver.py` starts the real service engine | Service authority, independent CLI/service lifetime, real socket lifecycle, process identity, crash/restart and persisted recovery | Nothing beyond the scenario and assertions actually made |
...
| `test_control_plane.py` direct `Conductor` tests | Engine semantic | Control-plane validation, workspace binding, checkpoints, lifecycle/publication rules, accepted integration state, reconciliation semantics | `invoke()` setup is a subprocess bootstrap helper; most assertions exercise direct engine methods |
...
`LiveService` launches an independent `python -m conductor --foreground` process, waits
...
| --- | --- | --- | --- |
| `ServiceEngine` | `cli.main()` constructs it for persistent service modes through `_service_engine`; `run_service()` hosts it; IPC dispatch receives it | KEEP - canonical current path | `src/conductor/cli.py`, `src/conductor/daemon.py`, `src/conductor/service.py` |
| `Conductor` | `cli.main()` constructs it for `init`, `render`, `check`, and `control`; it supplies bootstrap/presentation behavior | KEEP - supported bootstrap/internal semantic surface | `src/conductor/cli.py:359`, `src/conductor/cli.py:493-500` |
| `Application` | No production references remain after I4; the former `_service_engine()` substitution branch was retired | REMOVE - unsupported runtime injection seam | Replaced by direct `ServiceEngine` construction in `src/conductor/cli.py` |
| `ConductorApplication` | No production, test, package export, or documentation contract was found | REMOVE - compatibility residue | `src/conductor/application.py` deleted; no supported 0.5 import contract |
| `conductor`, `conductor --foreground`, `conductor --once` | Construct the canonical `_service_engine()` and call `run_service()` with the selected hosting mode | KEEP - canonical service commands | `src/conductor/cli.py`, `src/conductor/daemon.py` |
| `conductor stop` | Uses the authenticated IPC stop request and does not construct an engine | KEEP - canonical service control command | `src/conductor/cli.py`, `src/conductor/ipc_server.py` |
| `conductor status` / `plan` | Tries IPC first and uses a read-only guarded `ServiceEngine` fallback only when authority is absent | KEEP - read-only bootstrap/offline path | `src/conductor/cli.py:58-94` |
| `conductor retry` / `reconcile update-base` | Validate CLI input and submit IPC intentions; do not construct a mutable CLI engine | KEEP - canonical client path | `src/conductor/cli.py:103-179`, `src/conductor/ipc_server.py:47-78` |
| `ServiceEngine.retry()` / `reconcile_update_base()` | Direct internal engine methods, called by semantic tests and owner-side code; not called by CLI client dispatch | KEEP - intentional internal semantic seam | `src/conductor/runtime.py:5339-5425`; owner command handling at `src/conductor/runtime.py:4642-4649` |

### Construction Map

```text
python -m conductor / console script
  -> cli.main()
     -> init/render/check/control -> Conductor(read_only=True)
     -> status/plan -> IPC, or guarded read-only ServiceEngine fallback
     -> retry/reconcile -> IPC client only
      -> conductor -> background launcher -> conductor --foreground
...
audit above now resolves `Conductor`, `ServiceEngine`, and the persistent
service host.
...
| `test_service_engine_is_the_only_runtime_authority` | Application/Conductor/ServiceEngine alias identity assertion | Direct `ServiceEngine` construction and owner-thread/real-service authority tests prove behavior rather than retired alias identity |
| `test_application_import_does_not_import_cli` | Import-direction check for the deleted `conductor.application` module | No supported module remains; package entrypoint/import tests and canonical source imports cover current import direction |
