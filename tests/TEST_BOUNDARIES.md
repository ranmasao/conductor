...
| Production topology | `LiveService` starts `python -m conductor run`, real CLI subprocesses use the real socket, or `h1_driver.py` starts the real service engine | Service authority, independent CLI/service lifetime, real socket lifecycle, process identity, crash/restart and persisted recovery | Nothing beyond the scenario and assertions actually made |
...
`LiveService` launches an independent `python -m conductor run` process, waits
...
| --- | --- | --- | --- |
| `ServiceEngine` | `cli.main()` constructs it for `run` and `daemon` through `_service_engine`; `daemon.run_service()` hosts it; IPC dispatch receives it | KEEP - canonical current path | `src/conductor/cli.py`, `src/conductor/daemon.py`, `src/conductor/service.py` |
...
| `conductor run` | Constructs the canonical `_service_engine()` and calls `run_service(..., once=args.once)` | KEEP - canonical foreground runtime command | `src/conductor/cli.py:525-526` |
| `conductor daemon` | Constructs the same canonical `_service_engine()` and calls `run_daemon()`; `run_daemon()` delegates to `run_service()` | KEEP - reachable command/compatibility spelling | `src/conductor/cli.py:523-524`, `src/conductor/daemon.py:56-58` |
...
audit above now resolves `Conductor`, `ServiceEngine`, `run`, and `daemon`.
