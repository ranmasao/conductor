...
import conductor.ipc_server as ipc_server
from conductor.ipc_protocol import (
    MAX_PAYLOAD_BYTES,
    IPCProtocolError,
    encode_frame,
    encode_request,
...
from conductor.ipc_server import (
    UNIX_SOCKET_PATH_MAX_BYTES,
    UnixIPCServer,
    dispatch_mutation,
    dispatch_read_only,
)
from conductor.runtime import ConductorError, RetryCandidate
...
def test_ping_status_and_plan_work_over_unix_socket(running_server):
    ...
    assert ping.ok and ping.result == {"service": "conductor", "protocol_version": 1}
...
def test_default_like_linux_state_path_fits_socket_limit(tmp_path, monkeypatch):
    engine, _state = make_engine(tmp_path, monkeypatch)
    representative = Path("/home/example-user/.local/state/conductor")
    path = representative / "sockets" / f"{engine.ipc_socket_path.stem}.sock"

    assert len(str(path).encode()) <= UNIX_SOCKET_PATH_MAX_BYTES


def test_excessively_long_socket_path_fails_as_conductor_error(
    tmp_path, monkeypatch
):
    engine, _state = make_engine(tmp_path, monkeypatch)
    ...
    with pytest.raises(ConductorError, match="IPC socket path is too long"):
        server.start()
...
def test_same_project_second_daemon_cannot_disturb_first_socket(
    ...
    try:
        ...
        with pytest.raises(ConductorError, match="already running"):
...
def test_active_endpoint_fails_closed_without_unlinking_it(
    ...
    server_a.start()
    try:
        with pytest.raises(ConductorError, match="endpoint is active"):
            server_b.start()
        assert request(engine.ipc_socket_path, "1", "ping").ok
    finally:
        server_b.stop()
...
def test_unexpected_socket_path_objects_fail_closed(
    ...
    with pytest.raises(ConductorError, match="not a socket"):
...
def test_foreign_socket_directory_owner_fails_closed(
    ...
    with pytest.raises(ConductorError, match="directory has unsafe ownership"):
...
def test_foreign_socket_owner_fails_closed(
    ...
    try:
        with pytest.raises(ConductorError, match="path has unsafe ownership"):
            UnixIPCServer(engine, path).start()
    finally:
...
def test_wrong_peer_credential_is_rejected_without_stopping_server(
    ...
    server.start()
    monkeypatch.setattr(
        "conductor.ipc_server._peer_credentials_are_current_user", lambda _socket: False
    )
    connection = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
    try:
...
def test_interrupted_retry_candidate_is_admitted_and_recovered_end_to_end(
    ...
    try:
        ...
        if interactive:
            ...
            argv = ["conductor", "retry", "--env", str(engine.env_file)]
        else:
            argv = ["conductor", "retry", "T-1", "--env", str(engine.env_file)]
...
def test_matching_live_worker_rejects_retry_without_queueing(
    ...
    monkeypatch.setattr(
        "conductor.runtime.observe_worker_identity", lambda _: "matching-live"
    )

    with pytest.raises(ConductorError, match="worker is already running"):
...
def _request(method, payload=None):
    from conductor.ipc_protocol import IPCRequest
