...
from conductor.runtime import _capture_worker_identity, _todo_fingerprint
...
def test_accepted_integration_state_requires_idle_and_exact_identity():
    ...
    with pytest.raises(ConductorError, match="invalid accepted integration state"):
        runtime.ServiceEngine._validate_state_invariant(
            {"phase": "idle", "accepted_integration": {"ticket_id": "T-1"}}
        )
    with pytest.raises(ConductorError, match="only valid while idle"):
        runtime.ServiceEngine._validate_state_invariant(
...
def test_explicit_control_reconciliation_adopts_exact_divergent_history(
    ...
    conductor = runtime.ServiceEngine(config)
    product_before = git(working, "rev-parse", "HEAD").stdout.strip()
    remote_before = git(
        working, "ls-remote", "origin", "refs/heads/conductor/control"
    ).stdout.split()[0]

    assert conductor.reconcile_control(from_head, to_head) == 0
    ...
    evidence = f"refs/conductor/recovery/control/{from_head}-{to_head}"
    assert git(working, "rev-parse", "--verify", evidence).stdout.strip() == from_head
    assert conductor._ticket_store()
    assert state.exists()
...
def test_control_reconciliation_remote_change_is_side_effect_free(
    ...
    evidence = f"refs/conductor/recovery/control/{from_head}-{to_head}"
    original_validate = conductor._validate_control_reconcile_admission
    calls = 0
    ...
    def validate(from_value, to_value):
        ...
        if calls == 1:
            git(
                ...
                "--force",
                "origin",
                f"{from_head}:refs/heads/conductor/control",
            )

    monkeypatch.setattr(conductor, "_validate_control_reconcile_admission", validate)
    with pytest.raises(ConductorError, match="remote HEAD changed"):
        conductor.reconcile_control(from_head, to_head)
...
def test_control_reconciliation_refuses_unsafe_or_unnecessary_cases(
    ...
    if case == "wrong-from":
        ...
        to_head = from_head
        git(control, "push", "--force", "origin", f"{from_head}:conductor/control")
    elif case == "dirty":
        (control / "uncommitted.txt").write_text("dirty\n")
    elif case == "running":
        conductor._state["phase"] = "agent_running"
...
def test_unrelated_local_parent_blocks_lifecycle_reset(tmp_path, monkeypatch):
    ...
    with pytest.raises(ConductorError, match="simulated lifecycle interruption"):
        conductor.run_once()
    report = ExecutionReport.from_dict(conductor._state["pending_execution_report"])
    control = next((state / "worktrees").glob("*/control"))
    ...
    Conductor._apply_lifecycle_once(conductor, report)
    lifecycle = git(control, "rev-parse", "HEAD").stdout.strip()
    ...
    recovered = Conductor(config)
    with pytest.raises(
        ConductorError, match="parent is not on the fresh remote lineage"
    ):
        recovered.run_once()
    assert git(control, "rev-parse", "HEAD").stdout.strip() == lifecycle
    assert manual in git(control, "log", "--format=%H").stdout.splitlines()
    remote_head = git(
        tmp_path / "remote.git", "rev-parse", "refs/heads/conductor/control"
    ).stdout.strip()
...
def test_checkpointing_recovery_recognizes_existing_exact_checkpoint(
    ...
    if dirty_after_checkpoint:
        ...
        with pytest.raises(ConductorError, match="ambiguous checkpoint history"):
            recovered.run_once()
        assert (execution / "late-change.txt").read_text() == "must remain\n"
        assert not git(
            execution, "ls-remote", "origin", "refs/heads/conductor/work/T-1"
        ).stdout.strip()
        return
...
def test_service_recovers_publication_failure_without_rerunning_worker(
    ...
    original_publish = conductor._publish_execution_branch

    def publish(*args):
        nonlocal publication_calls
        publication_calls += 1
        if publication_calls == 1:
            raise ConductorError("simulated publication outage")
        return original_publish(*args)

    monkeypatch.setattr(conductor, "_run_worker", worker)
    monkeypatch.setattr(conductor, "_publish_execution_branch", publish)
    stop_event = threading.Event()
    thread = threading.Thread(
        target=lambda: conductor.serve(stop_event), daemon=True
    )
    thread.start()
...
def test_checkpoint_recovery_refuses_stale_product_before_side_effects(
    ...
    with pytest.raises(
        ConductorError, match="product checkout or remote changed"
    ):
        recovered.run_once()
...
def test_check_does_not_treat_orphan_worker_as_runtime_owner(
    ...
    try:
        identity = _capture_worker_identity(helper, conductor._state["execution_id"])
        conductor._save_state(
            "agent_running",
...
def test_control_fast_forward_logs_generation_change(tmp_path, monkeypatch, capsys):
    ...
    git(seed, "push", "origin", "HEAD:conductor/control")

    new_head, _remote_head = conductor._sync_control()
    output = capsys.readouterr().out
...
def test_engine_reconciliation_and_update_base_resumes(
    ...
    def worker(workspace, prompt):
        observed.append((workspace.path, prompt, conductor._state["execution_id"]))
    ...
    monkeypatch.setattr(conductor, "_run_worker", worker)
    assert conductor.run_once() == 1
    reconciliation = conductor._state["reconciliation"]
    ...
    assert (
        conductor.reconcile_update_base("T-1", reconciliation["observed_product"])
        == 0
    )
    assert conductor._state["reconciliation"]["status"] == "resolved"
    assert conductor._state["resume_required"]["status"] == "required"
    assert git(execution, "rev-parse", "HEAD^").returncode == 0

    resumed = Conductor(config)
    resumed_ids = []
    prompts = []
...
def test_engine_reconcile_update_base_conflict_preserves_original_checkpoint(
    ...
    with pytest.raises(ConductorError, match="manual/advanced reconciliation"):
        conductor.reconcile_update_base("T-1", reconciliation["observed_product"])
    assert conductor._state["reconciliation"]["status"] == "pending"
...
def test_engine_dirty_product_can_become_valid_update_base_target(
    ...
    assert conductor.reconcile_update_base("T-1", target) == 0
    assert conductor._state["reconciliation"]["status"] == "resolved"
    assert conductor._state["resume_required"]["status"] == "required"
...
def test_engine_still_dirty_product_blocks_update_base_without_rewrite(
    ...
    with pytest.raises(ConductorError, match="product checkout is dirty"):
        conductor.reconcile_update_base("T-1", reconciliation["original_base"])
    assert conductor._state["reconciliation"]["status"] == "pending"
...
def test_engine_still_detached_product_blocks_update_base_without_rewrite(
    ...
    execution = next((state / "worktrees").glob("*/work/T-1"))
    checkpoint = git(execution, "rev-parse", "HEAD").stdout.strip()
    with pytest.raises(ConductorError, match="wrong branch"):
        conductor.reconcile_update_base("T-1", reconciliation["original_base"])
    assert conductor._state["reconciliation"]["status"] == "pending"
    assert git(execution, "rev-parse", "HEAD").stdout.strip() == checkpoint
...
def test_stranded_agent_running_without_worker_identity_refuses_retry(
    ...
    conductor = Conductor(config)
    persist_agent_running(conductor, state)
    conductor._save_state("agent_running", worker_identity=None)
    calls = []
    monkeypatch.setattr(conductor, "_run_worker", lambda *_args: calls.append(True))

    with pytest.raises(ConductorError, match="ownership cannot be proven absent"):
        conductor.retry("T-1")
    assert calls == []
...
def test_startup_reconciles_absent_worker_as_process_loss(tmp_path, monkeypatch):
    ...
    workspace, _control = persist_agent_running(
        conductor, state, record_start=True
    )
    (workspace.path / "partial.txt").write_text("preserve\n")
    helper = subprocess.Popen(
        [sys.executable, "-c", "import time; time.sleep(60)"],
...
def test_startup_launch_window_remains_blocked_without_identity(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(working)
    conductor = Conductor(config)
    persist_agent_running(conductor, state, record_start=True)
    conductor._save_state(
    ...
    with pytest.raises(ConductorError, match="unsafe execution stage worker-launch"):
        recovered.run_once()
    assert recovered._state["phase"] == "agent_running"
...
def test_matching_live_worker_identity_refuses_retry_without_signaling(
    ...
    try:
        identity = _capture_worker_identity(helper, conductor._state["execution_id"])
        conductor._save_state("agent_running", worker_identity=identity.as_dict())
        calls = []
        monkeypatch.setattr(
            conductor, "_run_worker", lambda *_args: calls.append(True)
        )

        with pytest.raises(ConductorError, match="worker is still alive"):
            conductor.retry("T-1")
        assert calls == []
