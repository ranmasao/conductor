...
def main() -> int:
    ...
    if point == "merge_after_effect":
        ...
        from conductor.execution_workspace import ExecutionWorkspaceManager
        ...
        if point == "integration_before_effect":
            ...
            def git(repo, *git_args, **git_kwargs):
                ...
                is_control_commit = (
                    ...
                    and "Conductor integrate" in subject
                )
                is_control_push = (
                    repo == engine.control_worktree
                    and git_args[:1] == ("push",)
                    and any("refs/heads/conductor/control" in arg for arg in git_args)
                )
                if (
