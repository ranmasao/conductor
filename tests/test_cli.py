...
from conductor.ipc_client import IPCClientError
from conductor.ipc_client import request as ipc_request
from conductor.ipc_server import UnixIPCServer
from conductor.runtime import ExecutionPlan, GitObservation, StatusSnapshot
from conductor.runtime_locator import RuntimeAuthorityPresent, RuntimeLocator
...
def git_fixture(tmp_path, request):
    ...
    state = Path("/tmp") / (
        "conductor-test-" + hashlib.sha256(str(tmp_path).encode()).hexdigest()[:8]
    )
...
def _short_runtime_config(git_fixture):
    ...
    state = Path("/tmp") / f"conductor-f1-{suffix}"
    config = git_fixture["tmp"] / "conductor-f1.env"
...
def test_invalid_subcommand_argument_is_concise(monkeypatch, capsys):
    monkeypatch.setattr("sys.argv", ["conductor", "stop", "--definitely-invalid"])
    ...
    stderr = capsys.readouterr().err
    assert error.value.code == 2
    assert "conductor stop: unrecognized argument: --definitely-invalid" in stderr
    assert "conductor stop --help" in stderr
    assert "usage:" not in stderr


def test_missing_commands_are_concise(monkeypatch, capsys):
    monkeypatch.setattr("sys.argv", ["conductor", "--foreground", "--once"])
    with pytest.raises(SystemExit) as error:
        main()
    stderr = capsys.readouterr().err
    assert error.value.code == 2
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["conductor", "stop", "--help"], "usage: conductor stop"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ["plan", "--help"],
            ["Show what Conductor plans to do next.", "emit machine-readable JSON"],
        ),
        (
            ["retry", "--help"],
...
def test_public_help_surfaces_are_successful_and_useful(
    ...
    monkeypatch.setattr("sys.argv", ["conductor", *argv])
    ...
    output = capsys.readouterr().out
    assert error.value.code == 0
    assert f"usage: conductor {' '.join(argv[:-1])}".rstrip() in output
...
def test_check_rejects_missing_project_context(git_fixture):
    ...
    (working / ".conductor/project.md").unlink()
...
def test_fresh_init_requires_external_adoption_before_control_check(
    ...
    assert git(working, "status", "--porcelain").stdout != ""
    assert (working / ".conductor/project.md").is_file()
    ...
    assert "README.md" not in (working / ".conductor/project.md").read_text()
    assert invoke(fixture, "check").returncode == 1

    (working / ".conductor/project.md").write_text(
        '---\n"type": "conductor.project"\n"common":\n  - "README.md"\n---\n'
    )
    git(working, "add", ".")
    git(working, "commit", "-m", "adopt Conductor bootstrap")
    assert git(working, "status", "--porcelain").stdout == ""
...
def test_bare_cli_starts_background_service_and_stop_ends_it(git_fixture, monkeypatch):
    ...
    while time.monotonic() < deadline:
        ...
        if (
            "shutdown explicitly requested through conductor stop" in log
            and "orderly shutdown complete (stop_command)" in log
        ):
            break
        time.sleep(0.02)
    assert "shutdown explicitly requested through conductor stop" in log
...
def test_service_start_forms_are_idempotent_for_healthy_owner(
    ...
    try:
        ...
        assert repeated.stdout.strip() == "Conductor service is already running."
    finally:
...
def test_stop_without_service_is_conclusive(git_fixture):
    ...
    assert result.returncode == 1
    assert result.stderr.strip() == "conductor: service is not running"
...
def test_status_uses_daemon_ipc_without_fallback(
    ...
    monkeypatch.setattr(
        "conductor.cli._service_engine",
        lambda *_args, **_kwargs: pytest.fail("direct fallback used"),
    )
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "status", "--json", "--env", str(config)],
    )
...
def test_plan_uses_daemon_ipc_without_fallback(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "plan", "--json", "--env", str(config)],
    )
...
def test_blocked_dependency_status_uses_daemon_semantics(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "status", "--env", str(config)],
    )
...
def test_retry_uses_daemon_authority_and_never_constructs_cli_engine(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "retry", "T-1", "--env", str(_short_runtime_config(git_fixture))],
    )
...
def test_reconcile_refuses_without_daemon_without_constructing_engine(
    ...
    assert "start `conductor`" in stderr
...
def test_retry_refuses_connectable_socket_without_authority(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "retry", "T-1", "--env", str(config)],
    )
    try:
        assert main() == 1
        stderr = capsys.readouterr().err
...
def test_retry_interactive_candidates_are_rendered_and_selected_locally(
    ...
    monkeypatch.setattr("conductor.cli._interactive_terminal", lambda: True)
    monkeypatch.setattr("builtins.input", lambda _prompt: "1")
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "retry", "--env", str(_short_runtime_config(git_fixture))],
    )
...
def test_daemon_application_error_is_authoritative(
    ...
    def fail_status():
        raise ConductorError("authoritative daemon failure")
...
def test_subdirectory_requires_repository_root_without_daemon(
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", command, "--env", str(config)],
    )

    assert main() == 1
    assert "run conductor from repository root" in capsys.readouterr().err
...
def test_operational_cli_constructs_service_engine_directly(git_fixture, monkeypatch):
    ...
    monkeypatch.setattr("conductor.cli.ServiceEngine", FakeServiceEngine)
    monkeypatch.setattr(
        sys, "argv", ["conductor", "--once", "--env", str(git_fixture["config"])]
    )
    monkeypatch.chdir(git_fixture["working"])
...
def test_once_uses_shared_foreground_service_host(git_fixture, monkeypatch):
    ...
    monkeypatch.setattr("conductor.cli.run_service", host)
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "--once", "--env", str(git_fixture["config"])],
    )
    monkeypatch.chdir(git_fixture["working"])
...
def _worker_script(path):
    path.write_text(
        ...
        "attempts = os.environ.get('CONDUCTOR_TEST_ATTEMPTS')\n"
        "if attempts:\n"
        "    pathlib.Path(attempts).open('a').write('attempt\\n')\n"
        "print(json.dumps({'type': 'tool_use', 'part': {'type': 'tool', "
        "'tool': 'conductor_report', 'state': {'status': 'completed', "
...
def _add_service_ticket(engine):
    ...
    git(
        ...
        "push",
        "origin",
        "HEAD:refs/heads/conductor/control",
    )
...
def _h1_execution_service(git_fixture, monkeypatch, point):
    ...
    monkeypatch.setenv("CONDUCTOR_TEST_ATTEMPTS", str(attempts))
    monkeypatch.setenv("H1_CRASH_POINT", point)
    config = _h1_config(git_fixture)
    config.write_text(
...
def _retryable_service(git_fixture, monkeypatch):
    ...
    monkeypatch.setenv("CONDUCTOR_TEST_WORKER_PID", str(pid_file))
    monkeypatch.setenv("CONDUCTOR_TEST_ATTEMPTS", str(attempts))
    config = _h1_config(git_fixture)
    config.write_text(
        config.read_text().replace("OPENCODE_BIN=true", f"OPENCODE_BIN={worker}")
    ...
    try:
        service.wait_for(
            ...
            and _worker_body_started(
                Path(os.environ["CONDUCTOR_TEST_WORKER_PID"]),
...
def _long_worker_script(path):
    path.write_text(
        ...
        "        handle.flush()\n"
        "pathlib.Path(os.environ['CONDUCTOR_TEST_WORKER_PID']).write_text(str(os.getpid()))\n"
        "while True:\n"
        "    time.sleep(1)\n"
    )
    path.chmod(0o755)
...
def test_real_service_remote_exact_control_descendant_is_recognized(
    ...
    try:
        ...
        git(publisher_control, "fetch", "origin", "conductor/control")
        git(publisher_control, "reset", "--hard", "origin/conductor/control")
...
def test_real_service_remote_interleaving_before_control_commit_blocks(
    ...
    try:
        ...
        git(publisher_control, "commit", "-m", "Conductor integrate T-1")
        git(
            publisher_control,
            "push",
            "origin",
...
def test_cli_status_and_plan_render_fake_engine_without_runtime(
    ...
    monkeypatch.setattr("conductor.cli._service_engine", FakeServiceEngine)
    monkeypatch.chdir(git_fixture["working"])
    monkeypatch.setattr(
        sys, "argv", ["conductor", "status", "--env", str(git_fixture["config"])]
    )
    assert main() == 0
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "status", "--json", "--env", str(git_fixture["config"])],
    )
    assert main() == 0
    assert json.loads(capsys.readouterr().out)["plan"]["action"] == "none"

    monkeypatch.setattr(
        sys, "argv", ["conductor", "plan", "--env", str(git_fixture["config"])]
    )
    assert main() == 0
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["conductor", "plan", "--json", "--env", str(git_fixture["config"])],
    )
    assert main() == 0
...
def test_retry_refuses_without_daemon_without_constructing_engine(
    ...
    monkeypatch.setattr("conductor.cli._service_engine", FakeServiceEngine)
    monkeypatch.setattr(
        sys, "argv", ["conductor", "retry", "T-1", "--env", str(git_fixture["config"])]
    )
    assert main() == 1
    assert calls == []
...
def test_modern_bound_execution_requires_every_identity_field(phase):
    ...
    for field in (
        ...
        with pytest.raises(ConductorError, match="execution workspace binding"):
            Conductor._validate_state_invariant(incomplete)
