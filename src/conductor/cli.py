...
from conductor.daemon import run_daemon, run_service
...
def _retry_daemon(env_file: Path, ticket_id: str | None) -> int:
    ...
    try:
        if not locator.daemon_authority_present():
            raise ConductorError(
                "daemon is not running for this checkout; start `conductor daemon`"
            )
...
def _reconcile_daemon(env_file: Path, ticket_id: str, onto: str) -> int:
    try:
        ...
        if not locator.daemon_authority_present():
            raise ConductorError(
                "daemon is not running for this checkout; start `conductor run`"
            )
        result = request(
            locator.socket_path,
