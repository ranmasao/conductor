...
from conductor.daemon import run_service
from conductor.ipc_client import (
    IPCClientError,
    decode_plan,
    ...
    try:
        ...
        if ipc_error is not None:
            if ipc_error.application:
                raise ConductorError(str(ipc_error)) from ipc_error
            raise ConductorError(str(error)) from ipc_error
        ...
        try:
            ...
            raise ConductorError(str(decode_error)) from decode_error
    except RuntimeLocatorError as error:
        raise ConductorError(str(error)) from error
...
def _retry_daemon(env_file: Path, ticket_id: str | None) -> int:
    ...
    if ticket_id is not None and not ticket_id:
        raise ConductorError("retry ticket ID must be non-empty")
    try:
        if not locator.daemon_authority_present():
            raise ConductorError(
                "service is not running for this checkout; start `conductor`"
            )
...
def _stop_service(env_file: Path) -> int:
    try:
        ...
        raise ConductorError(str(error)) from error
    if not locator.daemon_authority_present():
        raise ConductorError("service is not running")
    try:
        response = request(locator.socket_path, "stop", mutable=True)
    except IPCClientError as error:
        raise ConductorError(str(error)) from error
    if set(response) != {"accepted"} or response["accepted"] is not True:
        raise ConductorError("service IPC returned invalid stop acknowledgement")
...
def _healthy_service(env_file: Path) -> bool:
    ...
    if response != {"service": "conductor", "protocol_version": 1}:
        raise ConductorError("service authority exists but its IPC health is invalid")
    return True


def _startup_fd() -> int | None:
    value = os.environ.get("CONDUCTOR_STARTUP_FD")
    if value is None:
        return None
    try:
        return int(value)
...
def _start_background(env_file: Path) -> int:
    ...
    environment = {
        **os.environ,
        "CONDUCTOR_STARTUP_FD": str(write_fd),
    }
    command = [
        sys.executable,
        "-m",
    ...
    try:
        ...
        os.close(read_fd)
        raise ConductorError(f"cannot start background service: {error}") from error
    finally:
        os.close(write_fd)
    ...
    try:
        ...
        raise ConductorError(f"service startup failed: {detail}; log: {log_path}")
    finally:
        os.close(read_fd)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("status", "plan"):
        command_parser = commands.add_parser(
            ...
            description=(
                ...
                if name == "status"
                else "Show what Conductor plans to do next."
            ),
        )
    ...
    reconcile_commands = reconcile_parser.add_subparsers(
        dest="reconcile_command", parser_class=ConductorArgumentParser
    )
...
def main() -> int:
    ...
    if args.command is None:
        ...
        try:
            if _healthy_service(env_file):
                print("Conductor service is already running.")
                return 0
            if args.foreground:
                return run_service(
    ...
    if args.command == "reconcile" and args.reconcile_command != "update-base":
        ConductorArgumentParser(prog="conductor reconcile").error(
