...
from conductor.runtime_locator import (
    RuntimeLocator,
    RuntimeLocatorError,
    read_env,
    repository_root,
...
class OperatorCommand:
    ...
    admission_error: ConductorError | None = None
...
class ServiceEngine:
    ...
    def _validate_retry_admission(self, ticket_id: str) -> None:
        ...
        if self.service_snapshot().worker_running:
            raise ConductorError("service worker is already running")
        if self._state.get("phase") == "agent_running":
            ...
            if stage not in {
                ...
                raise ConductorError("persisted execution is not currently retryable")
            return
        candidate_ids = {
    ...
    def _validate_reconcile_admission(self, ticket_id: str, onto: str) -> None:
        ...
        if not isinstance(reconciliation, dict) or reconciliation.get("status") != (
            ...
            raise ConductorError(f"ticket {ticket_id} has no pending reconciliation")
    ...
    def _validate_control_reconcile_admission(
        ...
        if remote.returncode:
            raise ConductorError(f"control remote branch not found: {remote_ref}")
    ...
    def _validate_control_reconcile_identities(
        ...
        for name, head in (("local", from_head), ("remote", to_head)):
            ...
            if result.returncode or result.stdout.strip() != head:
                raise ConductorError(f"control {name} identity is not an exact commit")
    ...
    def _validate_operator_admission(self, command: OperatorCommand) -> None:
        ...
        if command.method == "reconcile-control":
            ...
            return
        raise ConductorError(f"unsupported operator command: {command.method}")
    ...
    def _admission_result(command: OperatorCommand) -> dict[str, object]:
        ...
        if command.admission_result is None:
            raise ConductorError("service did not produce an operator admission result")
    ...
    def _mutable_receipt(self, request_id: str) -> dict[str, object] | None:
        with self._receipt_lock:
            ...
            if not isinstance(receipts, dict):
                raise ConductorError("invalid mutable request receipt state")
            ...
            if not isinstance(receipt, dict) or set(receipt) != {
                ...
                raise ConductorError("invalid mutable request receipt")
            if not all(
    ...
    def _reject_pending_operator_command_on_shutdown(self) -> None:
        with self._operator_command_lock:
            ...
            if command is None:
                return
            self._operator_command = None
            command.admission_error = ConductorError(
                "service is shutting down; runtime command was not admitted"
            )
            command.admission_event.set()
    ...
    def _validate_state_invariant(state: dict[str, object]) -> None:
        ...
        if reconciliation is not None:
            if not isinstance(reconciliation, dict) or reconciliation.get(
                ...
                raise ConductorError("invalid reconciliation state")
            ...
            if not all(
                ...
                raise ConductorError("invalid reconciliation identity")
        ...
        if accepted_integration is not None:
            ...
            if not isinstance(accepted_integration, dict) or set(
                ...
                raise ConductorError("invalid accepted integration state")
        if phase == "idle":
            return
    ...
    def _run_once(self) -> int:
        ...
        if accepted_integration is not None:
            ...
            if not isinstance(pending_control_head, str):
                raise ConductorError("invalid accepted integration control identity")
        ...
        if not product["stable"]:
            ...
            if existing_evidence.returncode == 0 and (
                ...
                raise ConductorError("reconciliation evidence ref is inconsistent")
            pinned = _git(
                self.repo,
                "update-ref",
                evidence_ref,
    ...
    def _recover_accepted_integration(self) -> int:
        ...
        if not isinstance(ticket_id, str) or not ticket_id:
            raise ConductorError("invalid accepted integration ticket identity")
    ...
    def _reconciliation_evidence_ref(ticket_id: str, execution_id: str) -> str:
        return f"refs/conductor/reconciliation/{ticket_id}/{execution_id}"
...
'''
    ...
    def _reconcile_control_owned(
        ...
        with authority:
            ...
            evidence = (
                "refs/conductor/recovery/control/"
                f"{from_head}-{to_head}"
            )
            existing = _git(
    ...
    def _reconcile_update_base_owned(
        ...
        with authority:
            ...
            if target_result.returncode:
                raise ConductorError("requested reconciliation target is not a commit")
            ...
            if preserved.returncode or preserved.stdout.strip() != reconciliation[
                ...
                raise ConductorError("preserved worker checkpoint evidence changed")
            manager = ExecutionWorkspaceManager(
                self.repo, self.execution_worktree_root, ticket_id
            )
            if (
                ...
                raise ConductorError("reconciliation workspace binding is invalid")
            workspace = self._execution_workspace_for_recovery()
