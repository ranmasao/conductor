...
def dispatch_read_only(engine: object, request: IPCRequest) -> dict[str, object]:
    ...
    if request.method == "ping":
        return {"service": "conductor", "protocol_version": 1}
...
class UnixIPCServer:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._serve, name="conductor-ipc", daemon=True
        )
        self._thread.start()
    ...
    def _prepare_socket_directory(self) -> None:
        ...
        if info.st_uid != os.geteuid():
            raise ConductorError(f"IPC socket directory has unsafe ownership: {parent}")
        try:
    ...
    def _prepare_endpoint(self) -> None:
        ...
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISSOCK(info.st_mode):
            raise ConductorError(f"IPC socket path is not a socket: {self.path}")
        if info.st_uid != os.geteuid():
            raise ConductorError(f"IPC socket path has unsafe ownership: {self.path}")
        ...
        if active:
            raise ConductorError(f"IPC socket endpoint is active: {self.path}")
        try:
            self.path.unlink()
        except OSError as error:
            raise ConductorError(
